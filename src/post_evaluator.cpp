...
void PostEvaluator::prepare(Graph& graph) const {
    graph.enable_derivative_explanations(options_.emit_dot);
}
std::vector<DerivativeCheck> PostEvaluator::run(
    ExpressionProgram& program, const Evaluator& evaluator, Span<const NamedQuery> queries,
    ...
    if (options_.emit_dot) write_dot(program, evaluator, queries);
    std::vector<DerivativeCheck> checks;
    if (options_.verify_derivatives) {
        for (std::size_t i = 0; i < queries.size(); ++i)
            if (queries[i].request.kind == EvaluationQuery::Derivative)
                checks.push_back(verify(program, queries[i], results[i], input_values));
    }
    return checks;
}
void PostEvaluator::write_dot(ExpressionProgram& program, const Evaluator& evaluator,
...
DerivativeCheck PostEvaluator::verify(ExpressionProgram& program, const NamedQuery& query,
                                       double analytic,
    ...
    if (!root_only.contains(output)) {
        ...
        Evaluator probe({root, output});
        Inputs inputs = program.make_inputs(probe, input_values);
